...
<body>
    <div class="container">
        <div class="header" style="margin-bottom: 30px;">
            <img src="{{ asset('images/pitoe.png') }}" alt="Kendhi Pitoe Park" height="150" width="150" class="img-responsive">
        ...
        <div class="flex-container" style="margin-bottom: 10px; text-align:right;">
            ...
            <div>Jml yg Dibeli</div>
            <div>Subtotal</div>
        </div>
        @foreach ($detilOrder as $do)
            <div class="flex-container" style="text-align: right;">
                <div style="text-align: left;"><b>{{ $do->nama_menu }}</b></div>
                <div>{{ $do->jumlah }} x {{ $do->price }} </div>
                <div><b>Rp.{{ number_format($do->jumlah * $do->price) }}</b></div>
            </div>
        @endforeach
        <hr>
        <div class="flex-container" style="text-align: right; margin-top: 10px;">
            ...
            <div style="text-align: right;">
                <ul>
                    <li>Rp {{ number_format($dataOrder->total_price) }} </li>
                </ul>
            </div>
        </div>
        <hr>
